...
class ItemInformation : InformationMenuAction() {
    ...
    override fun onAction(text: String, x: Double, y: Double) {
        ...
        item = when {
            text.matches(Regex(FixedEditorPattern.ITEM.pattern)) -> mcclass.valueOf(text.split(":")[0])!!
            text.toIntOrNull() != null || text.matches(Regex(FixedEditorPattern.ITEMID.pattern)) -> IdAble.byId(mcclass.cls, text.split(":")[0].toShort())!!
            else -> return
        }
